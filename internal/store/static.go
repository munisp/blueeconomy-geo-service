...
func (store *Store) UpsertVesselStatic(ctx context.Context, report StaticReport) error {
	if report.MMSI == "" {
		return errors.New("static report requires mmsi")
	}
	if report.EpfsType == "" {
	...
	if err != nil {
		return fmt.Errorf("begin static upsert: %w", err)
	}
	...
	err = tx.QueryRow(ctx, `SELECT static_report_id,
		...
		report.DraughtMillimetres, report.Destination, report.EpfsType).Scan(&currentID, &unchanged)
	if err != nil && !errors.Is(err, pgx.ErrNoRows) {
		return fmt.Errorf("read current static row mmsi %s: %w", report.MMSI, err)
	}
	if unchanged {
	...
	if currentID != "" {
		if _, err := tx.Exec(ctx, `UPDATE vessels_static SET valid_to = $2
			...
			return fmt.Errorf("close static row %s: %w", currentID, err)
		}
	}
	if _, err := tx.Exec(ctx, `INSERT INTO vessels_static (
		...
		return fmt.Errorf("insert static row mmsi %s: %w", report.MMSI, err)
	}
	return tx.Commit(ctx)
}
